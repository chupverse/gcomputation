summary.gccontinuous <- function (object, digits=4, ci.type=NULL, ci.level=0.95, unadjusted=TRUE, ...)
{
  ...
  cat("G-computation : \n")
  tmp <- matrix(c(mean(x$m0, na.rm=TRUE), sd(x$m0, na.rm=TRUE), mean(x$m0, na.rm=TRUE)/sd(x$m0, na.rm=TRUE), NA), nrow=1)
  colnames(tmp) <- c("Estimate", "Std. Error", "z value", "Pr(>|z|)")
  rownames(tmp) <- "M0"
  res <- tmp
  
  tmp <- matrix(c(mean(x$m1, na.rm=TRUE), sd(x$m1, na.rm=TRUE), mean(x$m1, na.rm=TRUE)/sd(x$m1, na.rm=TRUE), NA), nrow=1)
  rownames(tmp) <- "M1"
  res <- rbind(res,tmp)
  ...
  if (!is.null(ci.type)) {
    if (ci.type == "norm") {
      ci_vals <- matrix(c(mean(x$m0, na.rm=TRUE) - qnorm(1-(1-ci.level)/2, 0, 1)*sd(x$m0, na.rm=TRUE),
                          mean(x$m0, na.rm=TRUE) + qnorm(1-(1-ci.level)/2, 0, 1)*sd(x$m0, na.rm=TRUE),
                          mean(x$m1, na.rm=TRUE) - qnorm(1-(1-ci.level)/2, 0, 1)*sd(x$m1, na.rm=TRUE),
                          mean(x$m1, na.rm=TRUE) + qnorm(1-(1-ci.level)/2, 0, 1)*sd(x$m1, na.rm=TRUE),
    ...
    if (ci.type == "perc") {
      ci_vals <- matrix(c(quantile(x$m0, probs = (1-ci.level)/2, na.rm = TRUE),
                          quantile(x$m0, probs = 1-(1-ci.level)/2, na.rm = TRUE),
                          quantile(x$m1, probs = (1-ci.level)/2, na.rm = TRUE),
                          quantile(x$m1, probs = 1-(1-ci.level)/2, na.rm = TRUE),
  ...
  if (unadjusted == TRUE) {
    cat("Unadjusted : \n")
    tmp <- matrix(c(mean(x$m0.unadj, na.rm=TRUE), sd(x$m0.unadj, na.rm=TRUE), mean(x$m0.unadj, na.rm=TRUE)/sd(x$m0.unadj, na.rm=TRUE), NA), nrow=1)
    colnames(tmp) <- c("Estimate", "Std. Error", "z value", "Pr(>|z|)")
    rownames(tmp) <- "M0"
    res <- tmp
    
    tmp <- matrix(c(mean(x$m1.unadj, na.rm=TRUE), sd(x$m1.unadj, na.rm=TRUE), mean(x$m1.unadj, na.rm=TRUE)/sd(x$m1.unadj, na.rm=TRUE), NA), nrow=1)
    rownames(tmp) <- "M1"
    res <- rbind(res,tmp)
    ...
    if (!is.null(ci.type)) {
      if (ci.type == "norm") {
        ci_vals <- matrix(c(mean(x$m0.unadj, na.rm=TRUE) - qnorm(1-(1-ci.level)/2, 0, 1)*sd(x$m0.unadj, na.rm=TRUE),
                            mean(x$m0.unadj, na.rm=TRUE) + qnorm(1-(1-ci.level)/2, 0, 1)*sd(x$m0.unadj, na.rm=TRUE),
                            mean(x$m1.unadj, na.rm=TRUE) - qnorm(1-(1-ci.level)/2, 0, 1)*sd(x$m1.unadj, na.rm=TRUE),
                            mean(x$m1.unadj, na.rm=TRUE) + qnorm(1-(1-ci.level)/2, 0, 1)*sd(x$m1.unadj, na.rm=TRUE),
      ...
      if (ci.type == "perc") {
        ci_vals <- matrix(c(quantile(x$m0.unadj, probs = (1-ci.level)/2, na.rm = TRUE),
                            quantile(x$m0.unadj, probs = 1-(1-ci.level)/2, na.rm = TRUE),
                            quantile(x$m1.unadj, probs = (1-ci.level)/2, na.rm = TRUE),
                            quantile(x$m1.unadj, probs = 1-(1-ci.level)/2, na.rm = TRUE),
