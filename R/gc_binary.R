gc_binary <- function(formula, data, group, effect="ATE", model, param.tune=NULL, cv=30, boot.type="bcv",
  ...
  if (boot.mi == FALSE) {
    return(.gc_binary(formula=formula, data=data, group=group, effect=effect, model=model, param.tune=param.tune, cv=cv,
