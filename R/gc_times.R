gc_times <- function(formula, data, group, pro.time, effect="ATE", model, param.tune=NULL, cv=10, boot.type="bcv",
  ...
  if (boot.mi == FALSE) {
    return(.gc_times(formula=formula, data=data, group=group, pro.time, effect=effect, model=model, param.tune=param.tune, cv=cv,
                      boot.type=boot.type,boot.number=boot.number, boot.tune=boot.tune, progress=progress ,seed=seed))
  }
  if (boot.mi == TRUE) {
    return(.miboot_gc_times(formula=formula, data=data, group=group, pro.time, effect=effect, model=model, param.tune=param.tune, cv=cv,
                             boot.type=boot.type,boot.number=boot.number, boot.tune=boot.tune, progress=progress, seed=seed,
                             m=m, ...))
  }
}